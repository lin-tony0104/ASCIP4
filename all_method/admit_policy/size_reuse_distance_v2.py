'''
v2 改成使用lru追蹤 value值
具體:  使用一個history 紀錄最近來過的obj 
問題: value有負數，這會導致， 再負數時size越大越好，這不太對
'''
from .admit_BASE import admit_BASE
from lib.dequedict import DequeDict
from collections import defaultdict

#改寫DequeDict
class my_cache(DequeDict):
    '''
    保存cache內容
    使用region維護cache_value
    '''
    def __init__(self,cache_size):
        super().__init__()

        self.value=0#cache_value
        self.obj_num=0#cache中 obj數
    #
    def pushFirst(self, key, value):
        super().pushFirst(key, value)
        self.obj_num+=1
        self.value+=value.o_val       

    def _remove(self, key):
        value=super()._remove(key)
        self.obj_num-=1
        self.value-=value.o_val   
        

    def _push(self, key, value):
        super()._push(key, value)
        self.obj_num+=1
        self.value+=value.o_val   

 

    def get_avg_cache_value(self):
        if self.obj_num==0:
            return 0
        return self.value/self.obj_num



        

    

class SRDb_v2(admit_BASE):
    class entry():
        def __init__(self,obj,time):
            self.o_size=obj.o_size
            self.o_block=obj.o_block
            self.o_val=0
            self.o_time=time
            self.hit=False

    def __init__(self,cache_size):
        self.req_num=0
        self.cache_size=cache_size
        self.cache=my_cache(cache_size)
        self.history=DequeDict()
        
        self.time=0
# #wiki 超參數
#         self.region_size=1000
#         # self.region_size=100
#         # self.region_size=10
#         self.region=defaultdict(int)
#         self.alpha=0.2#  0:只看過往   1:只看現在

#超參數  twitter24
        self.region_size=1            #82.64
        # self.region_size=10           #82.66
        # self.region_size=100          #82.87
        # self.region_size=500            #84.27
        # self.region_size=1000         #86.32        ok
        # self.region_size=1500         #86.28 
        # self.region_size=2000         #86.23 
        # self.region_size=5000           #6.22

        self.region=defaultdict(int)
        # self.alpha=0.05#  0:只看過往   1:只看現在   86.67  
        # self.alpha=0.01#  0:只看過往   1:只看現在   86.78  ok
        # self.alpha=0.05#  0:只看過往   1:只看現在 86.67
        # self.alpha=0.07#  0:只看過往   1:只看現在 86.6
        self.alpha=0.1#  0:只看過往   1:只看現在  86.55    
        # self.alpha=0.2#  0:只看過往   1:只看現在  86.32
        # self.alpha=0.5#  0:只看過往   1:只看現在  85.69





    def request(self, obj):
        self.time+=obj.o_size
        r=obj.o_size//self.region_size
        self.req_num+=1

        self.curr_obj=self.entry(obj,self.time)

        
        if obj.o_block in self.history:
            reuse_distance=self.time-self.history[obj.o_block].o_time
            self.curr_obj.o_val=(self.cache_size-reuse_distance)/self.curr_obj.o_size
            if self.curr_obj.o_val<0:
                self.curr_obj.o_val=0
        else:
            self.curr_obj.o_val=0
            while(self.history.cached_count+obj.o_size>self.cache_size):
                self.history.popFirst()
        # print(self.curr_obj.o_val)
        self.history[obj.o_block]=self.curr_obj




        self.region[r]=(1-self.alpha)*self.region[r] + self.alpha*self.curr_obj.o_val
        



    def evict(self, victim):
        del self.cache[victim.o_block]


        

    def hit(self, obj):
        self.cache[obj.o_block]=self.curr_obj
        self.cache[obj.o_block].hit=True
        
    def addToCache(self, obj):
        self.cache[obj.o_block]=self.curr_obj


    def GET_DEBUG_MESSAGE(self):
        message="cache_value:"+str(self.cache.get_avg_cache_value())\
        +"  alpha: "+str(self.alpha)\
        +"  region_size: "+str(self.region_size)
        return message    

    def judge(self,obj):
        #有剩餘空間直接准入
        r=obj.o_size//self.region_size
        # if self.req_num<10000000:#warmup
        #     return True
        if self.curr_obj.o_size<(self.cache_size-self.cache.cached_count):
            return True
        else:
            # print(obj.o_val)
            # if self.curr_obj.o_val>self.cache.get_avg_cache_value():
            if self.region[r]>self.cache.get_avg_cache_value():
                return True
            else:
                return False