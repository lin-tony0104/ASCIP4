...
#tw        
    ...
    def request(self, obj):
        ...
        if obj.o_block in self.history:
            reuse_distance=self.time-self.history[obj.o_block].o_time #reuse_distance可能大於cache_size    因為obj在history裡的這段期間內 有其他req重用時並不會把 obj往前推。
            self.curr_obj.o_cost=reuse_distance*self.curr_obj.o_size 
        else:
            self.curr_obj.o_cost=self.cache_size*self.curr_obj.o_size
    ...
    def judge(self,obj):
        ...
        if self.curr_obj.o_size<(self.cache_size-self.cache.cached_count):
            ...
            p=np.exp(-self.region[r]/self.cache.get_avg_cache_cost())  #准入機率
            rand=np.random.rand()
            if self.region[r]<self.cache.get_avg_cache_cost() or rand<=p:
                return True
            else:
                return False
