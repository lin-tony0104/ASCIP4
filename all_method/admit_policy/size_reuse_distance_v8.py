...
class my_cache(DequeDict):
    ...
    def get_avg_cache_val(self):
        if self.obj_num==0:
            return 0
        return self.val/self.obj_num
...
#tw        
    ...
    def request(self, obj):
        ...
        if obj.o_block in self.history:
            ...
            self.curr_obj.o_val=1/(curr_reuse_distance*self.curr_obj.o_size) 
        else:
            curr_reuse_distance=self.cache_size
            self.curr_obj.o_val=1/(curr_reuse_distance*self.curr_obj.o_size)   #rd設為 cache_size(較大的值)
    ...
    def GET_DEBUG_MESSAGE(self):
        message="cache_val:"+str(self.cache.get_avg_cache_val())\
    ...
    def judge(self,obj):
        ...
        if self.curr_obj.o_size<(self.cache_size-self.cache.cached_count):
            ...
            if self.region[r]>=self.cache.get_avg_cache_val():
                return True
            else:
                return False
